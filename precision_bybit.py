...
for i, coin in enumerate(coin_list):
    data_points = requests.get(
        f"https://api.bybit.com/v5/market/kline?category=linear&symbol={coin}USDT&interval=60&limit=2"
    )

    if data_points.status_code != 200:
        continue

    result = data_points.json()["result"]
    if "list" not in result:
        continue

    price_samples = []
    for data_point in result["list"]:
...
with open("./precisions_bybit.csv", "w") as f:
